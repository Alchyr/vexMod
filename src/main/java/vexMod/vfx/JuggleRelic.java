...
import vexMod.patches.RenderOrderPatches;

public class JuggleRelic extends AbstractGameEffect {
    ...
    private static final float hoveramplificationX = 30 * Settings.scale; //this gets multiplied by the angle you hit the relic at to calculate the end velocity after touching it.
    private static final float hoveramplificationY = 30 * Settings.scale; //this, too.
    private static final float verticalbias = 5 * Settings.scale; //upwards momentum when touching the relic is additively increased by this number, just to make it easier.
    private static final int maxrotation = 40; //maximum rotational velocity in degree
    private static final int difficultyincrease = 1; //every counter increases gravity and decreases friction by this much %. Resets when it hits the ground.
    ...
    private static final float randombounce = 0.1F * Settings.scale;
    ...
    private int counter;
    private boolean assigncounter;

    public JuggleRelic(AbstractRelic ar, boolean assigncounter, float scaleAmount) {
        ...
        this.rotation = 0;
        this.currentX = this.ar.currentX;
        this.currentY = this.ar.currentY;
        this.velocityX = 0;
        this.velocityY = 0;
        ...
        RenderOrderPatches.RenderOrderPatch.jrs.add(this);
    }

    public JuggleRelic(AbstractRelic ar) {
        this(ar, false, 1.0F);
    }
    ...
    public void renderCorrectly(SpriteBatch sb) {
        if (this.currentY > upperboundary + 100F * Settings.scale) {
            ...
            sb.draw(ar.img, this.currentX - 64.0F, upperboundary - 64.0F, 64.0F, 64.0F, 128.0F, 128.0F, this.scale, this.scale, 90, 0, 0, 128, 128, false, false);
        }
        sb.draw(ar.img, this.currentX - 64.0F, this.currentY - 64.0F, 64.0F, 64.0F, 128.0F, 128.0F, this.scale * scaleAmount, this.scale * scaleAmount, this.rotation, 0, 0, 128, 128, false, false);
    }
