...
import vexMod.relics.PopTire;

public class BouncingRelic extends AbstractGameEffect {
    ...
    public BouncingRelic(AbstractRelic ar, boolean assigncounter, float scaleAmount) {
        ...
        RenderOrderPatches.RenderOrderPatch.brs.add(this);
    }

    public BouncingRelic(AbstractRelic ar) {
        this(ar, false, 1.0F);
    }
    ...
    public void update() {
        ...
        if (assigncounter) {
            if (ar instanceof PopTire) {
                if (this.counter > PopTire.highScore) {
                    PopTire.highScore = this.counter;
                    ar.updateDescription(AbstractDungeon.player.chosenClass);
                }
            }
