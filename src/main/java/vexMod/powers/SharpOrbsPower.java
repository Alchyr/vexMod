...
public class SharpOrbsPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public void atEndOfTurn(boolean isPlayer) { // At the start of your turn
        for (int i = 0; i < amount; i++) {
            for (int b = 0; b < AbstractDungeon.player.filledOrbCount(); b++) {
                AbstractDungeon.actionManager.addToBottom(new DamageAction(AbstractDungeon.getRandomMonster(), new DamageInfo(AbstractDungeon.player, 1, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.SLASH_HORIZONTAL));
            }
        }
    }
