    ...
    public void atEndOfTurn(boolean isPlayer) { // At the start of your turn
        if (AbstractDungeon.player.filledOrbCount() > 0) {
            for (int i = 0; i < amount; i++) {
                AbstractDungeon.actionManager.addToBottom(new SharpOrbsAction(AbstractDungeon.getMonsters().getRandomMonster((AbstractMonster) null, true, AbstractDungeon.cardRandomRng), new DamageInfo(AbstractDungeon.player, 1, DamageInfo.DamageType.THORNS), AbstractDungeon.player.filledOrbCount()));
            }
        }
